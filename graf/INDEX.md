...
| `DAG.{h,c}`   | LSM of `wh128` records under `.dogs/graf/` driven by `GRAFUpdate`.  Each `wh128` half (`key`, `val`) carries a 60-bit hashlet plus a 4-bit per-half type (`COMMIT=1`, `TREE=2`, `BLOB=3`).  Entry kinds = `(key.type, val.type)` pairs: `(COMMIT,COMMIT)` parent edge, `(COMMIT,TREE)` root-tree edge, `(TREE,TREE/BLOB/COMMIT)` parent-tree → child via `key.hl = tree_h60 ^ RAPHashSeed60(name)` (segment-name XOR; `GRAF_SEG_SEED`).  Lookups always scan the (type, hashlet) range and verify against keeper to defend against the rare 60-bit collision.  Hashlets are 60-bit (top of SHA-1) — same width keeper uses, so there's no prefix-lift step.  `DAGCommitTree` / `DAGParents` / `DAGTreeChildren` / `DAGAncestors` / `DAGAncestorsOfMany` / `DAGAllCommits` / `DAGTopoSort` are the public navigation surface |
